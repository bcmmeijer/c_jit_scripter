...
private:
    ...
    int io_redirect::secure_dup(int src) {
        int ret = -1;
        bool fd_blocked = false;
        do {
            ret = dup(src);
    ...
    void io_redirect::secure_pipe(int* pipes) {
        int ret = -1;
        bool fd_blocked = false;
        do {
#ifdef _MSC_VER
    ...
    void io_redirect::secure_dup2(int src, int dest) {
        int ret = -1;
        bool fd_blocked = false;
        do {
            ret = dup2(src, dest);
    ...
    void io_redirect::secure_close(int& fd) {
        int ret = -1;
        bool fd_blocked = false;
        do {
            ret = close(fd);
